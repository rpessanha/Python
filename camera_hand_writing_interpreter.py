...
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 640)     ##Change resolution of the camera
...
while(1):
    ...
    if not ret:
        break
    frame2 = cv2.cvtColor(frame, cv2.COLOR_HSV2BGR)     ##BGR to HLS conversion
    if cv2.waitKey(1) == ord('s'):
        break

    lb = np.array([0, 0, 59])               ##Masking parameters
    ub = np.array([255, 57, 123])
